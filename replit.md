...
- **AI Alt Text Generator** (`/alt-text-generator`) — Pro feature (1 free try per session). Uploads image to server → Gemini Vision API (tries `gemini-2.0-flash` → `gemini-2.5-flash` → `gemini-2.0-flash-lite` fallback chain) generates 3 SEO-optimised alt text variants + expert tips. Client-side canvas resize to max 900px JPEG before sending. Privacy disclosure shown. Server endpoint: `POST /api/generate-alt-text` (requires `GEMINI_API_KEY` secret). 2 AdBanner slots. Added to homepage (7th tool card, indigo), header Tools dropdown ("AI Alt Text"), footer, sitemap, RelatedTools component.
